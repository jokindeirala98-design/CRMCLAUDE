...
exports.buildDossierPdf = buildDossierPdf;
/**
...
const voltis_info_1 = require("./voltis-info");
// ── Paleta Voltis cobalto ────────────────────────────────────────────────────
const COBALT_BG = (0, pdf_lib_1.rgb)(0.067, 0.165, 0.392); // #11308C centro hero
const COBALT_DARK = (0, pdf_lib_1.rgb)(0.039, 0.125, 0.373); // #0A205F esquina
const COBALT_MID = (0, pdf_lib_1.rgb)(0.122, 0.278, 0.710); // #1F47B5
const SKY_TINT = (0, pdf_lib_1.rgb)(0.180, 0.357, 0.851); // #2E5BD9 arriba
const VOLTIS_BLUE = (0, pdf_lib_1.rgb)(0.122, 0.357, 1.0); // #1F5BFF
const VOLTIS_SKY = (0, pdf_lib_1.rgb)(0.725, 0.820, 1.0); // #B9D1FF
const VOLTIS_ICE = (0, pdf_lib_1.rgb)(0.918, 0.945, 1.0); // #EAF1FF
const WHITE = (0, pdf_lib_1.rgb)(1, 1, 1);
const WHITE_85 = (0, pdf_lib_1.rgb)(0.95, 0.95, 0.95);
const WHITE_70 = (0, pdf_lib_1.rgb)(0.85, 0.85, 0.92);
const WHITE_60 = (0, pdf_lib_1.rgb)(0.75, 0.80, 0.90);
const INK_DEEP = (0, pdf_lib_1.rgb)(0.043, 0.106, 0.243); // #0B1B3E
const GREEN_LIVE = (0, pdf_lib_1.rgb)(0.498, 1.0, 0.725); // #7fffb9
let MASCOT_PNG_BYTES_CACHE = null;
function loadMascotBytes() {
    if (MASCOT_PNG_BYTES_CACHE)
        return MASCOT_PNG_BYTES_CACHE;
    try {
        const filePath = path_1.default.join(process.cwd(), 'public', 'mascota-transparente.png');
        MASCOT_PNG_BYTES_CACHE = fs_1.default.readFileSync(filePath);
        return MASCOT_PNG_BYTES_CACHE;
    }
    catch {
        return null;
...
async function buildDossierPdf(args) {
    const url = (0, voltis_info_1.voltisPortalUrl)(args.token);
    ...
    const sans = await pdf.embedFont(pdf_lib_1.StandardFonts.Helvetica);
    const sansBold = await pdf.embedFont(pdf_lib_1.StandardFonts.HelveticaBold);
    const mono = await pdf.embedFont(pdf_lib_1.StandardFonts.Courier);
    const monoBold = await pdf.embedFont(pdf_lib_1.StandardFonts.CourierBold);
    // Página A4
    const A4_W = 595.28;
    const A4_H = 841.89;
    ...
    for (let i = 0; i < 6; i++) {
        page.drawCircle({
            ...
            size: 150 - i * 12,
            color: (0, pdf_lib_1.rgb)(1, 1, 1), opacity: 0.04,
        });
    }
    for (let i = 0; i < 6; i++) {
        page.drawCircle({
            x: A4_W * 0.10, y: 200,
            size: 200 - i * 14,
            color: (0, pdf_lib_1.rgb)(1, 1, 1), opacity: 0.025,
        });
    }
    ...
    cursorY -= 6;
    // ── Subtítulo (lede) ────────────────────────────────────────────────────
    const subLines = wrapText('Hemos preparado un espacio privado donde puedes ver, en cualquier momento, todo lo que pasa con tu energía: consumo, gasto y facturas. Sin contraseñas, sin apps, sin papeleo. Sólo abrir y leer.', sans, 10.5, heroMaxW);
    let subY = cursorY;
    ...
    page.drawRectangle({
        ...
        color: (0, pdf_lib_1.rgb)(0, 0, 0), opacity: 0.18,
    });
    // Fondo glass
    page.drawRectangle({
    ...
    page.drawRectangle({
        ...
        color: WHITE,
        borderColor: (0, pdf_lib_1.rgb)(0.122, 0.357, 1.0), borderWidth: 0.5, borderOpacity: 0.15,
    });
    ...
    features.forEach((f, i) => {
        ...
        page.drawRectangle({
            ...
            color: (0, pdf_lib_1.rgb)(0, 0, 0), opacity: 0.12,
        });
        // Card glass
        page.drawRectangle({
    ...
    page.drawText(voltis_info_1.VOLTIS_INFO.phone, {
        x: A4_W - M - sansBold.widthOfTextAtSize(voltis_info_1.VOLTIS_INFO.phone, 13),
        y: footY + 32, font: sansBold, size: 13, color: WHITE,
    });
    page.drawText(voltis_info_1.VOLTIS_INFO.email, {
        x: A4_W - M - sans.widthOfTextAtSize(voltis_info_1.VOLTIS_INFO.email, 9.5),
        y: footY + 16, font: sans, size: 9.5, color: WHITE_85,
    });
    page.drawText(`www.${voltis_info_1.VOLTIS_INFO.website}`, {
        x: A4_W - M - sans.widthOfTextAtSize(`www.${voltis_info_1.VOLTIS_INFO.website}`, 9.5),
...
function chunkUrl(url, maxChars) {
    const out = [];
 ...
 */
function drawFeatureGlyph(page, kind, x, y) {
    const w = 22, h = 18;
    const white = (0, pdf_lib_1.rgb)(1, 1, 1);
    if (kind === 'chart') {
