...
function EditableCell({
  ...
  if (!editable) {
    return <span className="text-xs text-on-surface truncate block px-2 py-1.5">{value || '-'}</span>
  }
  ...
  if (editing) {
    return (
      <input
        ...
        className="w-full text-xs bg-white border border-primary/40 rounded px-2 py-1 outline-none ring-1 ring-primary/20 text-on-surface"
      />
    )
  }
  ...
  return (
    <button
      ...
      className="w-full text-left text-xs text-on-surface truncate block px-2 py-1.5 rounded hover:bg-primary/5 transition-colors cursor-text"
      title={value || 'Click para editar'}
    >
      {value || <span className="text-on-surface-variant/50 italic">-</span>}
    </button>
  )
}
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-2xl border shadow-ambient-sm overflow-hidden ${
        isRejected ? 'border-error/30' : isSent ? 'border-secondary/20' : 'border-outline-variant/20'
      }`}
    >
      ...
      {isRejected && (
        <div className="flex items-center gap-2 px-4 py-2 bg-error/5 border-b border-error/10">
          ...
          <span className="text-xs font-semibold text-error">Rechazado — pulsa para reenviar</span>
        </div>
      )}
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-on-surface truncate">{p.client_name || 'Sin nombre'}</p>
            <p className="text-[11px] text-on-surface-variant font-mono mt-0.5 truncate">{p.cups || 'Sin CUPS'}</p>
          </div>
          {/* Action button */}
          {isRejected ? (
            <button
              ...
              title="Reenviar"
            >
              {saving ? <Loader2 className="w-5 h-5 animate-spin text-error" /> : <RefreshCw className="w-5 h-5 text-error" />}
            </button>
          ) : isSent ? (
            <div className="flex items-center gap-1.5">
              {onReject && (
                ...
                >
                  <XCircle className="w-4.5 h-4.5 text-error/60 hover:text-error" />
                </button>
              )}
            ...
            <button
              ...
              disabled={saving}
              className="flex-shrink-0 w-10 h-10 rounded-xl bg-surface-container-low hover:bg-secondary/10 flex items-center justify-center transition-all"
              title="Marcar como enviado"
            >
              {saving ? <Loader2 className="w-5 h-5 animate-spin text-primary" /> : <Circle className="w-5 h-5 text-on-surface-variant" />}
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
          {p.cif && (
            <div>
              <span className="text-[10px] text-on-surface-variant uppercase">CIF</span>
              <p className="text-xs text-on-surface">{p.cif}</p>
            </div>
          )}
          {p.tariff && (
            <div>
              <span className="text-[10px] text-on-surface-variant uppercase">Tarifa</span>
              <p className="text-xs text-on-surface">{p.tariff}</p>
            </div>
          )}
          {p.consumo_anual && (
            <div>
              <span className="text-[10px] text-on-surface-variant uppercase">Consumo</span>
              <p className="text-xs text-on-surface">{p.consumo_anual}</p>
            </div>
          )}
          {p.poblacion && (
            <div>
              <span className="text-[10px] text-on-surface-variant uppercase">Población</span>
              <p className="text-xs text-on-surface">{p.poblacion}</p>
            </div>
          )}
        </div>

        {/* Date */}
        <p className="text-[10px] text-on-surface-variant/60 mt-2">{fmtDate(p.requested_at)}</p>
      </div>
    </motion.div>
  )
}
    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="px-4 lg:px-6 pb-24 lg:pb-8 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          {/* Section tabs */}
          <div className="flex bg-surface-container-low rounded-xl p-1 w-full sm:w-auto">
            <button
            ...
            >
              ...
              {(pendingCount + rejectedCount) > 0 && (
                <span className={`ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                  rejectedCount > 0 ? 'bg-error/10 text-error' : 'bg-amber-100 text-amber-700'
                }`}>
            ...
            >
              Enviados
              {sentItems.length > 0 && (
                <span className="ml-1.5 text-[10px] bg-secondary/10 text-secondary px-1.5 py-0.5 rounded-full font-bold">
                  {sentItems.length}
                </span>
              )}
            </button>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-none">
              ...
              <input
                ...
                placeholder="Buscar..."
                className="w-full sm:w-48 pl-8 pr-3 py-2.5 text-xs bg-surface-container-low border-0 rounded-lg outline-none focus:ring-1 focus:ring-primary/30 text-on-surface placeholder:text-on-surface-variant/50"
              />
            </div>
            ...
            {section === 'pending' && pendingItems.length > 0 && (
              <button
                onClick={sendAll}
                disabled={sendingAll}
                className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold text-white bg-secondary hover:bg-secondary/90 rounded-lg transition-all shadow-sm disabled:opacity-50 whitespace-nowrap"
              >
            ...
            <div className="relative" ref={exportMenuRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2.5 text-xs font-semibold text-on-surface bg-surface-container-low hover:bg-surface-container-high rounded-lg transition-all"
              >
              ...
              <AnimatePresence>
                {showExportMenu && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-ambient-lg border border-outline-variant/20 overflow-hidden z-20 min-w-[180px]"
                  >
                    <button
                      onClick={() => { exportCSV(currentItems, section); setShowExportMenu(false) }}
                      className="w-full text-left px-4 py-2.5 text-xs hover:bg-surface-container-low transition-colors flex items-center gap-2"
                    >
        ...
        <div className="lg:hidden space-y-3">
          {loading ? (
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-on-surface-variant mt-2">Cargando...</p>
            </div>
        ...
        {/* ===== DESKTOP VIEW: Table ===== */}
        <div className="hidden lg:block bg-white rounded-2xl border border-outline-variant/20 shadow-ambient-sm overflow-hidden">
              ...
              <thead>
                <tr className="bg-surface-container-low/60">
                  {COLUMNS.map((col) => (
                    <th
                      key={col.key}
                      className={`${col.width} px-2 py-2.5 text-[10px] font-bold uppercase tracking-wider text-on-surface-variant border-b border-outline-variant/20 ${col.align === 'center' ? 'text-center' : 'text-left'}`}
                    >
                      {col.label}
                    </th>
                  ))}
                ...
                {loading ? (
                  <tr>
                    <td colSpan={COLUMNS.length} className="py-16 text-center">
                      <Loader2 className="w-6 h-6 animate-spin text-primary mx-auto" />
                      <p className="text-xs text-on-surface-variant mt-2">Cargando prescorings...</p>
                    </td>
                  </tr>
                ) : currentItems.length === 0 ? (
                  <tr>
                      ...
                      <tr
                        ...
                        className={`border-b transition-colors ${
                          isRejected
                            ...
                            : saving === p.id
                            ? 'bg-primary/3 border-outline-variant/10'
                            : rowIdx % 2 === 0
                            ? 'bg-white border-outline-variant/10 hover:bg-primary/3'
                            : 'bg-surface/40 border-outline-variant/10 hover:bg-primary/3'
                        }`}
                      >
                          ...
                          {isRejected ? (
                            ...
                            >
                              {saving === p.id
                                ...
                                : <RefreshCw className="w-5 h-5 text-error group-hover:text-error/80" />
                              }
                            </button>
                          ) : isSent ? (
                            <div className="inline-flex items-center gap-0.5">
                              <button
                                ...
                                disabled={saving === p.id}
                                className="p-0.5 rounded text-error/30 hover:text-error transition-all"
                                title="Marcar como rechazado"
                              >
                              ...
                              <div className="relative" title={`Enviado · ${days}d restantes`}>
                                <CheckCircle2 className="w-5 h-5 text-secondary" />
                                {days <= 2 && (
                                  <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-amber-400 border border-white text-[7px] font-bold text-white flex items-center justify-center">
                                    {days}
                                  </span>
                                )}
                              </div>
                            ...
                            >
                              {saving === p.id
                                ...
                                : <Circle className="w-5 h-5 text-on-surface-variant/40 group-hover:text-secondary" />
                              }
                            </button>
                          )}
                        </td>
                        ...
                        <td className="w-28 px-2 py-1.5">
                          <div className="flex flex-col">
                            <span className="text-xs text-on-surface-variant">{fmtDate(p.requested_at)}</span>
                            {isRejected && (
                              <span className="text-[9px] font-bold text-error bg-error/10 px-1.5 py-0.5 rounded mt-0.5 inline-block w-fit">
                                RECHAZADO
                              </span>
                            )}
                            {isSent && (
          ...
          {!loading && currentItems.length > 0 && (
            <div className="px-4 py-2.5 border-t border-outline-variant/10 bg-surface/30 flex items-center justify-between">
        ...
        {!loading && prescorings.length === 0 && (
          <div className="bg-primary/5 rounded-2xl p-4 lg:p-6 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-on-surface">Los prescorings se crean automaticamente</p>
...
function EmptyState({ section }: { section: 'pending' | 'sent' }) {
  return (
    <div className="text-center py-12 px-4">
      {section === 'pending' ? (
        <>
          <Circle className="w-10 h-10 text-on-surface-variant/30 mx-auto" />
          <p className="text-sm text-on-surface-variant mt-3">No hay prescorings pendientes</p>
        ...
        <>
          <CheckCircle2 className="w-10 h-10 text-on-surface-variant/30 mx-auto" />
          <p className="text-sm text-on-surface-variant mt-3">No hay prescorings enviados</p>
          <p className="text-xs text-on-surface-variant/60 mt-1">
