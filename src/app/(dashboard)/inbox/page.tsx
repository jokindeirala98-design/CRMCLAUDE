...
import {
  ...
  X, Zap, FileText, User, ArrowRight, Send, Trash2,
...
export default function InboxPage() {
  ...
  const loadTelegramInbox = useCallback(async () => {
    ...
    try {
      ...
      // Silently fail — telegram section just won't show
      setTelegramItems([])
    } finally {
  ...
  const handleSelectClient = (client: ClientOption) => {
    ...
    if (usingTelegram) {
      // Skip invoices step — go straight to analysis
  ...
  // Start Telegram processing: mark flag and let user pick client
  const startTelegramProcessing = () => {
    if (selectedTelegramIds.size === 0) return
    setUsingTelegram(true)
    setTelegramExpanded(false)
    // User is already on client step — they just pick the client now
  }

  // ═══ ANALYSIS ═══

  // Analyze Telegram files (from URLs)
  ...
  // Build review data from first analyzed invoice
  const buildReviewData = async (firstData: any) => {
  ...
  const submitSupply = async () => {
    ...
    try {
      ...
      if (existingSupply) {
        ...
        if (usingTelegram) await markTelegramProcessed()
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <div className="px-4 lg:px-8 py-6 max-w-xl mx-auto">
        ...
        {/* ── Telegram Banner (only on client step, only if items exist) ── */}
        {step === 'client' && !usingTelegram && telegramItems.length > 0 && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            className="mb-6 rounded-2xl border border-[#2AABEE]/20 overflow-hidden bg-surface-container-low">

            {/* Header — always visible */}
            <button onClick={() => setTelegramExpanded(!telegramExpanded)}
              className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#2AABEE]/5 transition-colors">
              <div className="w-9 h-9 rounded-xl bg-[#2AABEE]/10 flex items-center justify-center flex-shrink-0">
                <Send className="w-4 h-4 text-[#2AABEE]" />
              </div>
              <div className="flex-1 text-left">
                <p className="text-sm font-semibold text-on-surface">
                  {telegramItems.length} archivo{telegramItems.length !== 1 ? 's' : ''} de Telegram
                </p>
                <p className="text-[10px] text-on-surface-variant">Pendientes de procesar</p>
              </div>
              {telegramExpanded ? <ChevronUp className="w-4 h-4 text-on-surface-variant" /> : <ChevronDown className="w-4 h-4 text-on-surface-variant" />}
            </button>
            ...
            <AnimatePresence>
              {telegramExpanded && (
                <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }}
                  ...
                  {/* Select all */}
                  <div className="flex items-center justify-between px-4 py-1.5 border-t border-outline-variant/10">
                    <button onClick={selectAllTelegram}
                      className="text-[10px] font-medium text-[#2AABEE] hover:text-[#229ED9] transition">
                      {selectedTelegramIds.size === telegramItems.length ? 'Deseleccionar' : 'Seleccionar todo'}
                    </button>
                    <button onClick={loadTelegramInbox}
                      className="p-1 rounded-lg hover:bg-surface-container-high transition text-on-surface-variant">
                      <RefreshCw className={`w-3 h-3 ${loadingTelegram ? 'animate-spin' : ''}`} />
                    </button>
                  </div>
                    ...
                    {telegramItems.map(item => (
                      <div key={item.id}
                        className={`flex items-center gap-2.5 px-4 py-2 transition-colors ${
                          selectedTelegramIds.has(item.id) ? 'bg-[#2AABEE]/5' : ''
                        }`}>
                        <button onClick={() => toggleTelegramItem(item.id)}
                          className={`w-4.5 h-4.5 rounded border-2 flex items-center justify-center flex-shrink-0 transition ${
                            selectedTelegramIds.has(item.id) ? 'bg-[#2AABEE] border-[#2AABEE]' : 'border-outline-variant/40'
                          }`}>
                          {selectedTelegramIds.has(item.id) && <Check className="w-2.5 h-2.5 text-white" />}
                        </button>
                        <FileText className={`w-3.5 h-3.5 flex-shrink-0 ${item.file_type === 'pdf' ? 'text-red-400' : 'text-blue-400'}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-on-surface truncate">{item.file_name || 'Archivo'}</p>
                          <p className="text-[9px] text-on-surface-variant">{item.sender_name || 'Comercial'} · {formatTime(item.created_at)}</p>
                        </div>
                        <a href={item.file_url} target="_blank" rel="noopener noreferrer"
                          className="p-1 rounded hover:bg-surface-container-high transition text-on-surface-variant">
                          <Download className="w-3 h-3" />
                        </a>
                        <button onClick={() => deleteTelegramItem(item)}
                          className="p-1 rounded hover:bg-error/10 transition text-on-surface-variant hover:text-error">
                  ...
                  {selectedTelegramIds.size > 0 && (
                    <div className="flex gap-2 px-4 py-2.5 border-t border-outline-variant/10">
                      <button onClick={() => dismissTelegramItems(Array.from(selectedTelegramIds))}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-medium text-on-surface-variant hover:bg-surface-container-high transition">
                        <Trash2 className="w-3 h-3" /> Descartar
                      </button>
                      <button onClick={startTelegramProcessing}
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-[#2AABEE] hover:bg-[#229ED9] transition active:scale-[0.98]">
                        <Zap className="w-3 h-3" />
                        Analizar {selectedTelegramIds.size}
                      </button>
                    </div>
                  )}
                </motion.div>
        ...
        {/* ── Telegram mode indicator ── */}
        {usingTelegram && step === 'client' && (
          <div className="mb-4 flex items-center gap-2 px-3 py-2 bg-[#2AABEE]/10 rounded-xl border border-[#2AABEE]/20">
            ...
            <span className="text-xs font-medium text-on-surface flex-1">
              Selecciona cliente para {selectedTelegramIds.size} archivo{selectedTelegramIds.size !== 1 ? 's' : ''} de Telegram
            </span>
            <button onClick={() => { setUsingTelegram(false); setSelectedTelegramIds(new Set()) }}
              className="p-1 rounded-lg hover:bg-[#2AABEE]/10 transition">
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                  ...
                  <div className="bg-surface-container-low rounded-xl border border-primary/20 p-4 space-y-3">
                    ...
                    <input type="text" value={newClientName} onChange={e => setNewClientName(e.target.value)}
                      ...
                      autoFocus onKeyDown={e => { if (e.key === 'Enter') handleCreateClient() }} />
        ...
        {step === 'invoices' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={startManualAnalysis} disabled={files.length === 0}
        ...
        {step === 'review' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              ...
              <div className="space-y-1.5">
                {files.map(f => (
                  <div key={f.id} className="flex items-center gap-2 text-xs">
                    {f.status === 'success' ? <CheckCircle className="w-3.5 h-3.5 text-green-500" />
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => usingTelegram ? reset() : setStep('invoices')}
                className="px-4 py-2.5 rounded-xl text-sm text-on-surface-variant hover:bg-surface-container-high transition">
                Atrás
              </button>
              <button onClick={submitSupply} disabled={successCount === 0}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white gradient-primary disabled:opacity-40 transition hover:opacity-90 active:scale-[0.98]">
                {existingSupply ? <><Plus className="w-4 h-4" /> Añadir facturas</> : <><Zap className="w-4 h-4" /> Crear suministro</>}
              </button>
            </div>
          </motion.div>
        )}
