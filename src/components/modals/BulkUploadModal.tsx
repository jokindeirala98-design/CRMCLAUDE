          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-5">
              ...
              <div className="space-y-3">
                ...
                {/* Excel results */}
                {xlsxResults.length > 0 && (() => {
                  const ok  = xlsxResults.filter(r => r.ok)
                  const err = xlsxResults.filter(r => !r.ok)
                  return (
                    <div className="space-y-2">
                      {/* Summary bar */}
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-semibold text-ok">{ok.length} OK</span>
                          {err.length > 0 && (
                            <span className="text-xs font-semibold text-err">{err.length} fallaron ⚠️</span>
                          )}
                        </div>
                        <button onClick={() => { setXlsxFiles([]); setXlsxResults([]) }} className="text-[10px] text-brand hover:underline">Importar más</button>
                      </div>

                      {/* Failures first, prominently */}
                      {err.length > 0 && (
                        <div className="rounded-xl border border-err/30 bg-err-container/20 p-3 space-y-1.5">
                          <p className="text-[10px] font-bold text-err uppercase tracking-wide">Ficheros con error — vuelve a intentarlo</p>
                          {err.map((r, i) => (
                            <div key={i} className="flex items-start gap-2">
                              <AlertCircle className="w-3.5 h-3.5 text-err flex-shrink-0 mt-0.5" />
                              <div className="min-w-0">
                                <p className="text-xs font-medium text-ink truncate">{r.fileName}</p>
                                <p className="text-[10px] text-err">{r.error || 'Error desconocido'}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                        {ok.map((r, i) => (
                          <div key={i} className="rounded-lg px-3 py-2 text-xs border bg-ok-container/20 border-ok/20">
                            <div className="flex items-center gap-2">
                              <CheckCircle2 className="w-3 h-3 text-ok flex-shrink-0" />
                              <span className="font-mono text-[10px] text-ink-3 truncate flex-1">{r.cups}</span>
                              <span className="text-[10px] text-ink-3">{r.tarifa}</span>
                            </div>
